...
bool ImGui::ButtonExT(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, int page, int in, bool border, ImColor clr) {
	...
	ImVec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 3.0f, label_size.y + style.FramePadding.y * 2.0f);
	...
	if (page == in) {
		PushStyleColor(ImGuiCol_Text, ImVec4(207 / 255.f, 207 / 255.f, 207 / 255.f, 1));
...
bool ImGui::Checkbox(const char* label, bool* v) {
	...
	if (label_size.x > 0.0f) {
		...
		RenderText(text_bb.GetTL() - ImVec2(0, 5), label);
		PopStyleColor();
	}
...
bool ImGui::SliderBehavior2(const ImRect& frame_bb, ImGuiID id, const char* display_format, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags) {
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	const ImGuiStyle& style = g.Style;
...
bool ImGui::SliderFloat5(const char* label, float* v, float v_min, float v_max, const char* display_format, float power, float width) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const bool value_changed = SliderBehavior2(frame_bb, id, display_format, v, v_min, v_max, power, 2, ImGuiSliderFlags_None);
	...
	RenderText(ImVec2(frame_bb.Max.x - ImGui::CalcTextSize(value_buf).x, frame_bb.Min.y - 25), value_buf, value_buf_end);
...
bool ImGui::SliderInt5(const char* label, int* v, int v_min, int v_max, const char* display_format) {
	if (!display_format)
		display_format = "%.0f";
	float v_f = (float)*v;
	bool value_changed = SliderFloat5(label, &v_f, (float)v_min, (float)v_max, display_format, 1.0f);
	*v = (int)v_f;
	return value_changed;
}
...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg) {
		...
		if (!value_changed) {
			if (io.KeyAlt)
			{
				...
				for (auto i = VK_BACK; i <= VK_RMENU; i++) {
					if (GetAsyncKeyState(i) && i != VK_INSERT) {
						key = i;
						value_changed = true;
						ClearActiveID();
					}
	...
	if (label_size.x > 0)
		RenderText(ImVec2(total_bb.Min.x + 30, frame_bb.Min.y + style.FramePadding.y), label);
